...
export default function AdminDashboard() {
    ...
    const handleVerifyUser = async (userId: string | number, currentPlan: string, email: string, name?: string) => {
        ...
        try {
            ...
            sendActivationEmail(email, name || "Member", newPlan, expiry.toISOString());
    ...
    const handleVerifySubmit = async (e: React.FormEvent) => {
        ...
        if (verifyModal.user) {
            await handleVerifyUser(verifyModal.user.id, plan, verifyModal.user.email || verifyModal.user.username, verifyModal.user.name);
                                                ...
                                                {user.plan ? (
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {user.plan.startsWith('PENDING_') && (
                                                            <button
                                                                onClick={() => handleVerifyUser(user.id, user.plan, user.email || user.username, user.name)}
