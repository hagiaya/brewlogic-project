...
const createXenditInvoice = async (orderId, amount, email, description) => {
    const settings = await getConfig('payment_settings', { isProduction: false });
    const keys = settings.isProduction ? settings.production : settings.sandbox;

    // Fallback to env
    const secretKey = keys.secretKey || process.env.XENDIT_SECRET_KEY;
    ...
    const response = await fetch('https://api.xendit.co/v2/invoices', {
        ...
        body: JSON.stringify({
            ...
            success_redirect_url: `${process.env.VITE_APP_URL || 'http://localhost:5173'}/#success`, // Simple redirect back
...
app.post('/api/create-transaction', async (req, res) => {
    ...
    try {
        // Shorter Order ID for Xendit (max char limit sometimes applies or just cleaner)
        ...
        if (paymentMethod === 'manual') {
            ...
            newTx.token = invoice.id; // Store Invoice ID as token
            newTx.payment_url = invoice.invoice_url; // Store URL if we want
